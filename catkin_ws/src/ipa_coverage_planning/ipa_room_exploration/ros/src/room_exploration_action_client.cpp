...
int main(int argc, char **argv)
{
	...
	double robot_radius;
	priv_nh.param("robot_radius", robot_radius, 0.5);
	...
	{
		const std::string test_map_path = ros::package::getPath("ipa_room_segmentation") + "/envs/";
		image_path = test_map_path + "pole.png";
		/* std::string env_pack_path;
		priv_nh.param<std::string>("env_pack", env_pack_path, "ipa_room_segmentation");
		std::string file_name;
		priv_nh.param<std::string>("image", file_name, ".pgm");
		std::string map_name;
		priv_nh.param<std::string>("robot_env", map_name, "pole");
