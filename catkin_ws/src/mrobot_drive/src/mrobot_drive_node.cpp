...
{
  ros::init(argc, argv, "mrobot_drive_node"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
...
void turn_on_robot::Publish_ImuSensor()
{
  ...
  Imu_Data_Pub.header.frame_id = imu_frame_id; //IMU corresponds to TF coordinates, which is required to use the robot_pose_ekf feature pack 
...
void turn_on_robot::Control()
{
  while(ros::ok())
  {
                                       ...
                                       //通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      ROS_INFO_STREAM("Data received success");
    } 
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<bool> ("ifSend", ifSend, false);
  private_nh.param<std::string>("usart_port_name",  usart_port_name,  "/dev/ttyUSB0"); //Fixed serial port number //固定串口号
  private_nh.param<int>        ("serial_baud_rate", serial_baud_rate, 9600); //Communicate baud rate 115200 to the lower machine //和下位机通信波特率115200
  private_nh.param<std::string>("odom_frame_id",    odom_frame_id,    "odom_link");      //The odometer topic corresponds to the parent TF coordinate //里程计话题对应父TF坐标
  ...
  private_nh.param<std::string>("imu_frame_id",    imu_frame_id,    "imu_link"); //IMU topics correspond to TF coordinates //IMU话题对应TF坐标
